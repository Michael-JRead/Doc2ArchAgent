...
4. If `provenance.yaml` exists, read it for confidence data and level restrictions
...
Write `architecture/<system-id>/diagrams/layout-plan.yaml` following the schema below.
...
The layout plan is the intermediate artifact that all renderers read. Write it to `architecture/<system-id>/diagrams/layout-plan.yaml`.
